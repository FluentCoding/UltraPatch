// original tests from https://github.com/json-patch/json-patch-tests/blob/master/tests.json
const tests = [
  {
...
Bun.file(`${__dirname}/tests.json`).write(
  JSON.stringify(
    tests
      .filter((test) => "expected" in test)
      .map((test) => [
        test.comment ?? "unnamed test",
        test.doc,
        test.patch,
        test.expected,
      ]),
...
console.info("Successfully generated tests json");
